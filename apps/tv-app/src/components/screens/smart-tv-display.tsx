 ...
 */
export function SmartTvDisplay({ layout, onHlsChannelOpen, onWebChannelOpen }: SmartTvDisplayProps) {
  ...
  useEffect(() => {
    if (!token) return;
    fetchRotationAds();
    const interval = setInterval(fetchRotationAds, 60_000); // Refresh every 1 min
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          position: 'absolute',
          inset: 'var(--tv-safe-y, 1.2vh) var(--tv-safe-x, 1.2vw)',
        }}
      >
        {/* Top bar */}
        <TopBar
