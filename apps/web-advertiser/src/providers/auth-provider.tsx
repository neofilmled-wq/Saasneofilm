...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = typeof window !== 'undefined' ? localStorage.getItem('neofilm_adv_token') : null;
    const storedUser = typeof window !== 'undefined' ? localStorage.getItem('neofilm_adv_user') : null;
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('neofilm_adv_token', data.accessToken);
      localStorage.setItem('neofilm_adv_user', JSON.stringify(authUser));
      localStorage.setItem('neofilm_adv_refresh', data.refreshToken);
    }
  }, []);
  ...
  const signup = useCallback(async (data: { email: string; password: string; firstName: string; lastName: string }) => {
    const res = await fetch(`${API_URL}/auth/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...data, interfaceType: 'ADVERTISER' }),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('neofilm_adv_token', resData.accessToken);
      localStorage.setItem('neofilm_adv_user', JSON.stringify(authUser));
      localStorage.setItem('neofilm_adv_refresh', resData.refreshToken);
    }
  }, []);
  ...
  const setTokensFromCallback = useCallback(async (accessToken: string, refreshToken: string, _isNew: boolean) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('neofilm_adv_token', accessToken);
      localStorage.setItem('neofilm_adv_user', JSON.stringify(authUser));
      localStorage.setItem('neofilm_adv_refresh', refreshToken);
    }
  }, []);
  ...
  const logout = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('neofilm_adv_token');
      localStorage.removeItem('neofilm_adv_user');
      localStorage.removeItem('neofilm_adv_refresh');
    }
  }, []);
